...
router.post('/', upload.single('file'), async function(req, res) {
  try {
    ...
    var agrupadas = agruparProximos(paradas, 0.03);
    var resultado = await otimizarRotaAvancada(agrupadas);
    var lucroEstimado = paradas.length * 12.75;
...
function processarPlanilha(texto) {
  ...
  var cabecalho = linhas[0].split(',').map(function(h) { return h.trim().replace(/"/g, '').toLowerCase(); });
